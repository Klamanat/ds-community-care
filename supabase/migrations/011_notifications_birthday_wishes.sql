-- ============================================================
-- 011_notifications_birthday_wishes.sql
-- 1. Birthday notification → only shown to the birthday person
-- 2. Add "received birthday wish" notifications (last 7 days)
-- ============================================================
...
returns table (
  ...
  with raw as (
    ...
    -- Birthdays this month: everyone EXCEPT yourself
    select
      'bday_' || e.id                                           as id,
      'birthday'                                                as type,
      e.name || ' มีวันเกิดเดือนนี้! 🎂'                      as title,
      '🎂 ' || coalesce(e.bd_date, '')                          as msg,
      now()                                                     as ts,
      'bday'                                                    as target,
      'linear-gradient(135deg,#F9A8D4,#EC4899)'                as color
    from employees e
    where e.bd_date is not null
      and e.month_idx = p_month_idx
      and e.name != p_emp_name         -- ← exclude yourself

    union all

    -- Birthday wishes received (last 7 days)
    select
      'wish_' || bw.id                                          as id,
      'bday_wish'                                               as type,
